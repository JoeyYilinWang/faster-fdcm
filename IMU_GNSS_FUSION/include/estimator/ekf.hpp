...
public:
   ...
   * @param last_imu k-1时刻的imu数据
   * @param curr_imu k时刻的imu
   */
  void predict(ImuDataConstPtr last_imu, ImuDataConstPtr curr_imu) {
    ...
    imu_model_.propagate_state(last_imu, curr_imu, last_state, *state_ptr_); // *state_ptr_中内部的状态量此时是预测的
    imu_model_.propagate_state_cov(last_imu, curr_imu, last_state, *state_ptr_); // *state_ptr_中内部的协方差矩阵是预测的
  }
  ...
  /**
   * @brief update kalman convariance matrix
   */
  template <class H_type, class R_type, class K_type>
